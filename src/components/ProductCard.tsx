...
export default function ProductCard({ product, onSelect }: ProductCardProps) {
  ...
  return (
    ...
    >
      <div className="relative aspect-square mb-6 overflow-hidden bg-white border border-line">
        <img 
        ...
        {isOutOfStock && (
           <div className="absolute inset-0 flex items-center justify-center">
             <span className="text-[10px] uppercase tracking-widest font-bold bg-white/90 px-4 py-2 border border-charcoal/10 shadow-sm">Esgotado</span>
           </div>
        )}
      </div>

      <div className="space-y-1">
        <h3 className="serif text-[14px] font-normal leading-tight text-charcoal">{product.name}</h3>
        ...
        <button 
          onClick={(e) => {
            e.stopPropagation();
            if (!isOutOfStock && !isLimitReached) addToBag(product);
          }}
          ...
          className={`mt-3 inline-block text-[10px] uppercase tracking-[0.5px] border-b pb-0.5 transition-all ${
            ...
            : 'border-charcoal/30 opacity-70 hover:opacity-100'
          }`}
        >
